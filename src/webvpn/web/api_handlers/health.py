...
from webvpn.entities.application import (
    ApplicationResponse,
    HealthResponse,
    PongResponse,
)
...
@health_rt.get("/ping/", response_model=ApplicationResponse[PongResponse])
async def pong(connection: PGConnectionDepends):
    ...
    session = await AuthSessionRepo.get(
        GetSessionByUserUUIDStragegy(user.uuid), connection
    )
    print(session)
