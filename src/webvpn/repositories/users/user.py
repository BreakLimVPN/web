...
from webvpn.repositories.users.get_users_strategy import (
    GetFullUserStrategy,
    GetUserByNameStragegy,
    GetUserStrategy,
)
...
class UserRepository:
    async def create(self, username, password, connect: Connection) -> User:
        ...
        if not username or not password:
            raise HTTPException(
                status_code=400, detail="Заполните поля username/password"
            )

        database_user = await self.get(GetUserByNameStragegy(username), connect)
        if database_user:
    ...
    async def get_full_user(
        self, strategy: GetFullUserStrategy, connect: Connection
    ) -> FullUser | None:
        record = await connect.fetch(strategy.query(), strategy.identifier)
