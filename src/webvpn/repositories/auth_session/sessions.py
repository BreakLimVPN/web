...
from webvpn.repositories.auth_session.get_session_strategy import (
    GetSessionByUserUUIDStragegy,
    GetSessionStrategy,
)
...
class AuthSessionRepository:
    async def create(self, user: User, connect: Connection) -> Session | None:
        old_session: Session | None = await self.get(
            GetSessionByUserUUIDStragegy(user.uuid), connect
        )
        ...
        return await self.get(GetSessionByUserUUIDStragegy(user.uuid), connect)

    async def get(
        self, strategy: GetSessionStrategy, connect: Connection
    ) -> Session | None:
